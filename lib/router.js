...
Router.map(function() {
  this.route('homePage', {
   ...
   this.route('revenues', {
        ...
        action: function(){
            this.render('revenues');
        }
    });
  
    this.route('sumbitMonth', {
        path: '/forms/submit',
        template: 'data-entry',
        before: function () {
            if (!Meteor.user()) {
    ...
    this.route('formsList', {
        ...
        action: function(){
            this.render('formsList');
        }
    });

  this.route('data-entry', {
        path: '/forms/dataentry',
        template: 'data-entry',
        before: function () {
            if (!Meteor.user()) {
                this.render('accesdenied');
    ...
    this.route('monthPage', {
        path: '/forms/:_id',
        template: 'monthPage',
        data: function() { return Kpimonth.findOne(this.params._id); },
        before: function () {
            if (!Meteor.user()) {
                this.render('accesdenied');
                this.stop();
